  ...
  public class CContour {
        ...
        for( int s = 0 ; s < stripCount ; s++ ) {
          ...
          for( int p = 0 ; p < ptCount ; p++ ) {
            ...
            if( ( ( ( boxH.easting - boxC.easting ) * ( stripList[s][p].northing - boxC.northing ) )
                    - ( ( boxH.northing - boxC.northing ) * ( stripList[s][p].easting - boxC.easting ) ) ) < 0 ) {
              continue;
            }

            if( ( ( ( boxC.easting - boxB.easting ) * ( stripList[s][p].northing - boxB.northing ) )
                    - ( ( boxC.northing - boxB.northing ) * ( stripList[s][p].easting - boxB.easting ) ) ) < 0 ) {
              continue;
            }

            if( ( ( ( boxB.easting - boxF.easting ) * ( stripList[s][p].northing - boxF.northing ) )
                    - ( ( boxB.northing - boxF.northing ) * ( stripList[s][p].easting - boxF.easting ) ) ) < 0 ) {
              continue;
            }

            if( ( ( ( boxF.easting - boxH.easting ) * ( stripList[s][p].northing - boxH.northing ) )
                    - ( ( boxF.northing - boxH.northing ) * ( stripList[s][p].easting - boxH.easting ) ) ) < 0 ) {
              continue;
            }
          ...
          for( int s = 0 ; s < stripCount ; s++ ) {
            ...
            for( int p = 0 ; p < ptCount ; p++ ) {
              ...
              if( ( ( ( boxG.easting - boxE.easting ) * ( stripList[s][p].northing - boxE.northing ) )
                      - ( ( boxG.northing - boxE.northing ) * ( stripList[s][p].easting - boxE.easting ) ) ) < 0 ) {
                continue;
              }

              if( ( ( ( boxE.easting - boxA.easting ) * ( stripList[s][p].northing - boxA.northing ) )
                      - ( ( boxE.northing - boxA.northing ) * ( stripList[s][p].easting - boxA.easting ) ) ) < 0 ) {
                continue;
              }

              if( ( ( ( boxA.easting - boxD.easting ) * ( stripList[s][p].northing - boxD.northing ) )
                      - ( ( boxA.northing - boxD.northing ) * ( stripList[s][p].easting - boxD.easting ) ) ) < 0 ) {
                continue;
              }

              if( ( ( ( boxD.easting - boxG.easting ) * ( stripList[s][p].northing - boxG.northing ) )
                      - ( ( boxD.northing - boxG.northing ) * ( stripList[s][p].easting - boxG.easting ) ) ) < 0 ) {
                continue;
              }
    ...
    public void DistanceFromContourLine( vec3 pivot ) {
      ...
      if( ptCount > 8 ) {
        ...
        Classes.CPath.VehicleLimitSteerAngle( mf.vehicle.maxSteerAngle, steerAngleCT );

        //Reduce the radius, if necessary, of the steering circle on the display to 500
        Classes.CPath.RadiusLimitSteeringCircleDisplay( ppRadiusCT );
        ...
        Classes.CPath.VelocityLimitSteerAngle( mf.vehicle.wheelbase, mf.pn.speed, mf.vehicle.maxAngularVelocity, steerAngleCT );
