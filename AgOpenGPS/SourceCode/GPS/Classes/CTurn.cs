  ...
  public class CTurn {
    ...
    public void FindClosestTurnPoint( bool isYouTurnRight, vec3 fromPt, double headAB ) {
      ...
      for( int b = 1 ; b < 1500 ; b += 2 ) {
        ...
        if( turnArr[0].IsPointInTurnWorkArea( pt ) ) {
          for( int t = 1 ; t < FormGPS.MAXBOUNDARIES ; t++ ) {
            if( !mf.bnd.bndArr[t].isSet )
              continue;
            if( mf.bnd.bndArr[t].isDriveThru )
              continue;
      ...
      for( int p = 0 ; p < ptCount ; p++ ) {
        if( ( ( ( boxB.easting - boxA.easting ) * ( turnArr[closestTurnNum].turnLine[p].northing - boxA.northing ) )
                - ( ( boxB.northing - boxA.northing ) * ( turnArr[closestTurnNum].turnLine[p].easting - boxA.easting ) ) ) < 0 ) {
          continue;
        }

        if( ( ( ( boxD.easting - boxC.easting ) * ( turnArr[closestTurnNum].turnLine[p].northing - boxC.northing ) )
                - ( ( boxD.northing - boxC.northing ) * ( turnArr[closestTurnNum].turnLine[p].easting - boxC.easting ) ) ) < 0 ) {
          continue;
        }

        if( ( ( ( boxC.easting - boxB.easting ) * ( turnArr[closestTurnNum].turnLine[p].northing - boxB.northing ) )
                - ( ( boxC.northing - boxB.northing ) * ( turnArr[closestTurnNum].turnLine[p].easting - boxB.easting ) ) ) < 0 ) {
          continue;
        }

        if( ( ( ( boxA.easting - boxD.easting ) * ( turnArr[closestTurnNum].turnLine[p].northing - boxD.northing ) )
                - ( ( boxA.northing - boxD.northing ) * ( turnArr[closestTurnNum].turnLine[p].easting - boxD.easting ) ) ) < 0 ) {
          continue;
        }
    ...
    public void BuildTurnLines() {
      ...
      for( int j = 1 ; j < FormGPS.MAXBOUNDARIES ; j++ ) {
        ...
        if( !mf.bnd.bndArr[j].isSet || mf.bnd.bndArr[j].isDriveThru || mf.bnd.bndArr[j].isDriveAround )
          continue;
    ...
    public void DrawTurnLines() {
      ...
      for( int i = 0 ; i < FormGPS.MAXBOUNDARIES ; i++ ) {
        if( !mf.bnd.bndArr[i].isSet && mf.bnd.bndArr[i].isDriveAround )
          continue;
        //turnArr[i].DrawTurnLine();
        {
          ...
          for( int h = 0 ; h < ptCount ; h++ )
            GL.Vertex3( mf.turn.turnArr[i].turnLine[h].easting, mf.turn.turnArr[i].turnLine[h].northing, 0 );
