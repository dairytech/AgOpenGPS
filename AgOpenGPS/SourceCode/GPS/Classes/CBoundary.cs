...
namespace AgOpenGPS {
  public class CBoundary {
    ...
    public void FindClosestBoundaryPoint( vec3 fromPt, double headAB ) {
      ...
      for( int i = 0 ; i < FormGPS.MAXHEADS ; i++ ) {
        ...
        for( int p = 0 ; p < ptCount ; p++ ) {
          if( ( ( ( boxB.easting - boxA.easting ) * ( bndArr[i].bndLine[p].northing - boxA.northing ) )
                  - ( ( boxB.northing - boxA.northing ) * ( bndArr[i].bndLine[p].easting - boxA.easting ) ) ) < 0 ) {
            continue;
          }

          if( ( ( ( boxD.easting - boxC.easting ) * ( bndArr[i].bndLine[p].northing - boxC.northing ) )
                  - ( ( boxD.northing - boxC.northing ) * ( bndArr[i].bndLine[p].easting - boxC.easting ) ) ) < 0 ) {
            continue;
          }

          if( ( ( ( boxC.easting - boxB.easting ) * ( bndArr[i].bndLine[p].northing - boxB.northing ) )
                  - ( ( boxC.northing - boxB.northing ) * ( bndArr[i].bndLine[p].easting - boxB.easting ) ) ) < 0 ) {
            continue;
          }

          if( ( ( ( boxA.easting - boxD.easting ) * ( bndArr[i].bndLine[p].northing - boxD.northing ) )
                  - ( ( boxA.northing - boxD.northing ) * ( bndArr[i].bndLine[p].easting - boxD.easting ) ) ) < 0 ) {
            continue;
          }
      ...
      closestBoundaryPt.easting = -20000;
      closestBoundaryPt.northing = -20000;
