  ...
  public partial class FormGPS {
    ...
    private void oglMain_Paint( object sender, PaintEventArgs e ) {
      if( isGPSPositionInitialized ) {
        ...
        if( isGridOn )
          worldGrid.DrawWorldGrid( camera.gridZoom );
        ...
        if( isSkyOn ) {
          ...
          if( camera.camPitch < -60 ) {
            ...
            GL.TexCoord2( 0, 0 );
            GL.Vertex2( winRightPos, 0.0 ); // Top Right
            GL.TexCoord2( 1, 0 );
            GL.Vertex2( winLeftPos, 0.0 ); // Top Left
            GL.TexCoord2( 0, 1 );
            GL.Vertex2( winRightPos, hite * oglMain.Height ); // Bottom Right
            GL.TexCoord2( 1, 1 );
            GL.Vertex2( winLeftPos, hite * oglMain.Height ); // Bottom Left
        ...
        if( isLightbarOn ) {
          ...
          if( ct.isContourBtnOn ) {
            ...
            txtDistanceOffABLine.Visible = true;
            //lblDelta.Visible = true;
            if( curve.distanceFromCurrentLine == 32000 )
              curve.distanceFromCurrentLine = 0;

            DrawLightBar( oglMain.Width, oglMain.Height, curve.distanceFromCurrentLine * 0.1 );
            if( ( curve.distanceFromCurrentLine ) < 0.0 ) {
              txtDistanceOffABLine.ForeColor = Color.Green;
              if( isMetric )
                dist = ( (int)Math.Abs( curve.distanceFromCurrentLine * 0.1 ) ) + " ->";
              else
                dist = ( (int)Math.Abs( curve.distanceFromCurrentLine / 2.54 * 0.1 ) ) + " ->";
              txtDistanceOffABLine.Text = dist;
            } else {
              txtDistanceOffABLine.ForeColor = Color.Red;
              if( isMetric )
    ...
    private void oglBack_Paint( object sender, PaintEventArgs e ) {
      ...
      for( int j = 0 ; j < vehicle.numOfSections ; j++ ) {
        if( section[j].sectionLookAhead > rpHeight )
          rpHeight = section[j].sectionLookAhead;
        ...
        for( int j = 0 ; j < vehicle.numOfSections ; j++ ) {
          ...
          if( section[j].sectionLookAhead > 0 ) {
            ...
            if( bnd.bndArr[0].isSet ) {
              ...
              if( end > vehicle.rpWidth - 1 )
                end = vehicle.rpWidth - 1;
    ...
    private void oglZoom_Paint( object sender, PaintEventArgs e ) {
      ...
      if( curve.isCurveSet ) {
        ...
        if( ptC > 0 ) {
          ...
          GL.Begin( PrimitiveType.LineStrip );
          for( int h = 0 ; h < ptC ; h++ )
            GL.Vertex3( curve.curList[h].easting, curve.curList[h].northing, 0 );
          GL.End();
        }
      }
    ...
    public void CalculateMinMax() {
      ...
      if( maxFieldX == -9999999 | minFieldX == 9999999 | maxFieldY == -9999999 | minFieldY == 9999999 ) {
        ...
        if( dist > dist2 )
          maxFieldDistance = ( dist );
        else
          maxFieldDistance = ( dist2 );

        if( maxFieldDistance < 200 )
          maxFieldDistance = 200;
        if( maxFieldDistance > 19900 )
