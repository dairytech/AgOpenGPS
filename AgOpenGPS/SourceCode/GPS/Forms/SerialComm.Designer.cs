...
namespace AgOpenGPS {
  public partial class FormGPS {
      ...
      if( isInAutoDrive ) //Is in Auto Drive Mode enabled
      {
        if( !ast.isInFreeDriveMode ) {
          ...
          if( recPath.isDrivingRecordedPath ) {
            mc.pedalControl |= 0b11000000;
          }
          //make it not go - and with zero
          else {
            mc.pedalControl &= 0b00111111;
          }
        ...
        mc.pedalControl |= 0b01000000;
        mc.pedalControl &= 0b01111111;
      }
    ...
    public void AutoSteerSettingsOutToPort() {
      Byte[] dataForUDP = {
                0x7F, 0xFC, // PGN
                Properties.Settings.Default.setAS_Kp,
                Properties.Settings.Default.setAS_Ki,
                Properties.Settings.Default.setAS_Kd,
                Properties.Settings.Default.setAS_Ko,
                Properties.Settings.Default.setAS_steerAngleOffset,
                Properties.Settings.Default.setAS_minSteerPWM,
                Properties.Settings.Default.setAS_maxIntegral,
                Properties.Settings.Default.setAS_countsPerDegree
            };
      ...
      if( spAutoSteer.IsOpen ) {
        try {
          spAutoSteer.Write( dataForUDP, 0, dataForUDP.Length );
        } catch( Exception e ) {
    ...
    public void AutoSteerControlOutToPort() {
      ...
      Byte[] dataForUDP = {
                ...
                sectionByte,
                (byte)(pn.speed * 4),
                (byte)(guidanceLineDistanceOff>>8),
                (byte)(guidanceLineDistanceOff),
                (byte)(guidanceLineSteerAngle>>8),
                (byte)(guidanceLineSteerAngle)
            };
      SendUDPMessage( dataForUDP );
    ...
    public void MachineControlOutToPort() {
      byte sectionByte = 0;
      //check if super section is on
      if( section[vehicle.numOfSections].isSectionOn ) {
        sectionByte = 0xff;
    ...
    public void RateOutToPort() {
      UInt16 sectionWord = 0;
      //check if super section is on
      if( section[vehicle.numOfSections].isSectionOn ) {
        sectionWord = 0xffff;
      } else {
      ...
      Byte[] dataForUDP = {
                ...
                (byte)(sectionWord>>8),
                (byte)(sectionWord),
                (byte)((Int16)(rcd.rateSetPointLeft * 100.0) >> 8),
                (byte)(rcd.rateSetPointLeft * 100.0),
                (byte)((Int16)(rcd.rateSetPointRight * 100.0) >> 8),
                (byte)(rcd.rateSetPointRight * 100.0),
                (byte)(pn.speed * 4),
            };
...
#warning example PGN Section control!
      //int set = 1;
      //int reset = 2046;
    ...
    public void RateRelayOutToPort( byte[] items, int numItems ) {
      ...
      if( spRelay.IsOpen ) {
        try {
          spRelay.Write( items, 0, numItems );
        } catch( Exception e ) {
...
#warning implement reading from serial!
