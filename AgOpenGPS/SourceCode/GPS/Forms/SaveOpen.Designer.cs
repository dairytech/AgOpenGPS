      ...
      if( ofd.ShowDialog() == DialogResult.OK ) {
        ...
        using( StreamReader reader = new StreamReader( ofd.FileName ) ) {
          try {
            ...
            words = line.Split( ',' );
            if( words[0] == "Empty" )
              Properties.Vehicle.Default.setVehicle_goalPointLookAheadUturnMult = 0.7;
            else
            ...
            words = line.Split( ',' );
            if( words[0] == "Empty" )
              Properties.Vehicle.Default.setVehicle_lookAheadMinimum = 3;
            else
            ...
            words = line.Split( ',' );
            if( words[0] == "Empty" )
              Properties.Vehicle.Default.setVehicle_lookAheadDistanceFromLine = 1.2;
            else
            ...
            for( int i = 0 ; i < FormGPS.MAXFUNCTIONS ; i++ )
              int.TryParse( words[i], out seq.seqEnter[i].function );

            sentence = Properties.Vehicle.Default.seq_ActionEnter;
            words = sentence.Split( ',' );
            for( int i = 0 ; i < FormGPS.MAXFUNCTIONS ; i++ )
              int.TryParse( words[i], out seq.seqEnter[i].action );
            ...
            words = sentence.Split( ',' );
            for( int i = 0 ; i < FormGPS.MAXFUNCTIONS ; i++ )
              int.TryParse( words[i], out seq.seqExit[i].function );

            sentence = Properties.Vehicle.Default.seq_ActionExit;
            words = sentence.Split( ',' );
            for( int i = 0 ; i < FormGPS.MAXFUNCTIONS ; i++ )
              int.TryParse( words[i], out seq.seqExit[i].action );
            ...
            for( int k = 0 ; k < MAXBOUNDARIES ; k++ ) {
              ...
              if( numPoints > 0 ) {
                ...
                if( bnd.bndArr[k].area > 0 )
                  bnd.bndArr[k].isSet = true;
                else
                  bnd.bndArr[k].isSet = false;
              }
              if( reader.EndOfStream )
                break;
            }
