...
#warning do something more inteligent to block the autosteer controller
    ...
    private void UpdateFixPosition() {
      ...
      if( distanceCurrentStepFix <= minFixStepDist ) {
        for( currentStepFix = 0 ; currentStepFix < totalFixSteps ; currentStepFix++ ) {
          ...
          if( fixStepDist > minFixStepDist ) {
            ...
            if( currentStepFix < ( totalFixSteps - 1 ) )
              currentStepFix++;
            isFixHolding = false;
            break;
          } else
      ...
      if( recPath.isDrivingRecordedPath )
        recPath.UpdatePosition();

      // If Drive button enabled be normal, or just fool the autosteer and fill values
      if( !ast.isInFreeDriveMode ) {
        // add side draft angle
        if( ahrs.isHeadingPAOGI ) {
          guidanceLineSteerAngle = (Int16)( guidanceLineSteerAngle + ( pn.nRoll * ( (double)Properties.Settings.Default.setAS_Kd ) * 4.166666 ) );
        } else {
      ...
      if( ( spRelay.IsOpen ) | ( isUDPSendConnected ) ) {
        DoRemoteSectionSwitch();
      }
    ...
    private void CalculatePositionHeading() {
      ...
      if( ( ahrs.isHeadingBrick | ahrs.isHeadingBNO | ahrs.isHeadingPAOGI ) && mc.gyroHeading != 9999 ) {
        ...
        if( gyroCorrected > glm.twoPI )
          gyroCorrected -= glm.twoPI;
        if( gyroCorrected < 0 )
          gyroCorrected += glm.twoPI;
      ...
      if( vehicle.toolFarLeftSpeed < vehicle.toolFarRightSpeed )
        ...
        sectionTriggerStepDistance = vehicle.toolFarRightSpeed / metersPerSec;
...
metersPerSec * triangleResolution * 2.0 + 1.0;
    ...
    private void AddBoundaryAndPerimiterPoint() {
      ...
      for( int i = 0 ; i < MAXBOUNDARIES ; i++ )
        isInner |= bnd.bndArr[i].isOkToAddPoints;
    ...
    public void CalculateSectionLookAhead( double northing, double easting, double cosHeading, double sinHeading ) {
      ...
      for( int j = 0 ; j < vehicle.numOfSections ; j++ ) {
        ...
        if( leftLook > rightLook )
          section[j].sectionLookAhead = leftLook;
        else
          section[j].sectionLookAhead = rightLook;

        if( section[j].sectionLookAhead > 190 )
          section[j].sectionLookAhead = 190;

        //Doing the slow mo, exceeding buffer so just set as minimum 0.5 meter
        if( currentStepFix >= totalFixSteps - 1 )
          section[j].sectionLookAhead = 5;
      }//endfor
      ...
      if( section[vehicle.numOfSections - 1].sectionLookAhead > 0 )
        vehicle.toolFarRightSpeed = rightSpeed * 0.05;
      else
        vehicle.toolFarRightSpeed = 0;

      //save left side, 0 if going backwards, in meters/sec convert back from pixels/m
      if( section[0].sectionLookAhead > 0 )
        vehicle.toolFarLeftSpeed = vehicle.toolFarLeftSpeed * 0.05;
      else
        vehicle.toolFarLeftSpeed = 0;
    }
    ...
    private void InitializeFirstFewGPSPositions() {
      if( !isFirstFixPositionSet ) {
        ...
        if( startCounter > ( totalFixSteps / 2 ) )
    ...
    private void DoRemoteSectionSwitch() {
      ...
      if( rcd.SectMainSWFromArduino != rcd.SectMainSWFromArduinoOld ) {    //Rate SW                
                                                                           //witch Rate left/right? left = 0
        if( ( rcd.SectMainSWFromArduino & 16 ) == 16 ) {
          rcd.RateStepsLeft = false;
        } else {
          rcd.RateStepsLeft = true;
        }
        //Rate direction 1 = up
        if( ( rcd.SectMainSWFromArduino & 32 ) == 32 ) {
          rcd.RateUp = true;
        } else {
          rcd.RateUp = false;
        }
        ...
        if( ( rcd.SectMainSWFromArduino & 4 ) == 4 ) {
          rcd.RateStepsFromArudino = 1;
        }
        if( ( rcd.SectMainSWFromArduino & 8 ) == 8 ) {
          rcd.RateStepsFromArudino = rcd.RateStepsFromArudino + 2;
        }
        if( ( rcd.RateStepsFromArudino > rcd.RateStepsFromArduinoOld ) && ( ( rcd.RateStepsLeft != rcd.RateStepsLeftOld ) | ( rcd.RateUp != rcd.RateUpOld ) ) ) {   //same side/direction then reduce by last value to prevent to much effect
          int Val = 0;
          ...
          rcd.RateStepsFromArduinoOld = rcd.RateStepsFromArudino;
